...
import Data_2401051012 from '@/dummy/Data_2401051012';

export default function Modal_4() {
  return (
    <>
      <div className={`flex flex-row gap-[0] w-[794px] h-full`}>
        ...
        <div className={`Fc flex-1 bg-white p-[80px_60px] overflow-auto`}>
          ...
          <section className={`mt-[56px]`}>
            <FormLabel_1>컨텐츠 제작 목적</FormLabel_1>
            <div className={`mt-[40px] space-y-[20px]`}>
              {Data_2401051012.map((obj, idx) => {
                const { 제목, 설명 } = obj;
                ...
                return (
                  <Fragment key={idx}>
                    <div>
                      <label
                        className={`FormLabelBox !items-start !justify-start`}
                      >
                        <input className={`RadioButton_1 Md`} type={`radio`} />
                        <div className={`LabalUi`}>
                          <div>{제목}</div>
                          <div
                            className={`text-[#3D3C42] text-[14px] font-[400]`}
                          >
                            {설명}
                          </div>
                        </div>
